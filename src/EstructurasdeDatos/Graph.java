...
public abstract class Graph {
    ...
    public abstract LinearElements<Edge> adjacentTo(int i);
    @Override
    public String toString(){
        String r = "";
        for (int i = 1; i<nVertices();i++){
            r += "\nVértice " + i + " ";
            LinearElements<Edge> pointer = adjacentTo(i);
            if (pointer.isEmpty()){
                r += "sin Adyacentes.\n";
            }else{
                r += "con Adyacentes: \n";
    ...
    protected void toArrayDFS(int origin, int res[]) {
        ...
        LinearElements<Edge> l = adjacentTo(origin);
    ...
    protected void toArrayBFS(int origin, int res[]) {
        ...
        while (!q.isEmpty()) {
            int u = q.deleteFirst();
            LinearElements <Edge> l = adjacentTo(u);
            for (l.setFirst(); !l.isLast(); l.keepMoving()) {
                Edge a = l.getMove();
                if (visited[a.dest] == 0) {
